...
void GameScene1_3::CopyAct(Input& input)
{
	if (m_pItems)
	{
			...
			if (m_pPlayer->GetAnimIdx() > 3)
			{
				...
				if (isHitItem)
				{
					...
					if (m_pItems->GetItemState() == ItemState::Burning)//バーニングのアイテムでの変身
					{
						...
						m_pItems = nullptr;
						//ここでアップデートを変える
						update_ = &GameScene1_3::CopyingUpdate;
...
void GameScene1_3::CopyingUpdate(Input&)
{
	//変身中の処理
	m_pPlayer->AnimFrameUpdate();
	m_pPlayer->AnimFrameUpdate();
...
{
	NormalDraw();
	if (update_ == &GameScene1_3::DyingUpdate)m_pPlayer->DyingDraw(camera);
	const auto& wsize = Application::GetInstance().GetWindowSize();
...
void GameScene1_3::NormalDraw()
{
	...
	if (update_ == &GameScene1_3::CopyingUpdate)//変身中の暗転
	{
		//灰色にしたいものをここに
		m_pBg->Draw(camera);
		m_doors->Draw(camera);
	...
	else
	{
		...
		if (update_ != &GameScene1_3::DyingUpdate) m_pPlayer->Draw(camera);//死に際の描画は別
		for (auto& enemy : m_pEnemyWizards)//ペンギン
		{
			if (enemy) enemy->Draw(camera);
		}
