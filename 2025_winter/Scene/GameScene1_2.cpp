...
void GameScene1_2::CopyAct(Input& input)
{
	if (m_pItems)
	{
...
void GameScene1_2::NormalUpdate(Input& input)
{
	...
	if (input.IsTriggered("ok"))
	{
		...
		draw_ = &GameScene1_2::FadeOutDraw;
	}
#endif // _DEBUG
...
void GameScene1_2::ShakingUpdate(Input&)
{
	UpdateCamera(camera, m_pPlayer);
...
void GameScene1_2::FadeDraw()
{
	...
	NormalDraw();
	if (update_ == &GameScene1_2::DyingUpdate)m_pPlayer->DyingDraw(camera);
	const auto& wsize = Application::GetInstance().GetWindowSize();
...
void GameScene1_2::NormalDraw()
{
	...
	else
	{
		...
		if (update_ != &GameScene1_2::DyingUpdate) m_pPlayer->Draw(camera);//死に際の描画は別
		for (auto& enemy : m_pEnemyWizards)//ペンギン
		{
			if (enemy) enemy->Draw(camera);
		}
